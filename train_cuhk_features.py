...
import nets.deep_sort.network_definition_features as net
...
class Cuhk(object):
    ...
    def __init__(self, dataset_dir, use_features=False, num_validation_y=0.1, seed=1234):
    ...
    def read_train(self):
        filenames, ids, camera_indices, _ = cuhk.read_train_split_to_str(
            self._dataset_dir, self._use_features)
...
def main():
    arg_parser = train_app.create_default_argument_parser("cuhk")
    arg_parser.add_argument(
        "--dataset_dir", help="Path to CUHK dataset directory.",
        default="resources/CUHK-evaluation-master")
    args = arg_parser.parse_args()
    dataset = Cuhk(args.dataset_dir, use_features=True, num_validation_y=0.1, seed=1234)
    ...
    if args.mode == "train":
        ...
        train_app.train_loop(
            ...
            num_images_per_id=2, image_shape=IMAGE_SHAPE, is_features=True, **train_kwargs)
    elif args.mode == "eval":
